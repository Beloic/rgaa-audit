                      ...
                      <div className="px-4 py-3 border-t border-gray-100">
                        <div className="space-y-2">
                          <div className="flex items-center justify-between">
                            <span className="text-xs text-gray-600">Audits ce mois</span>
                            <span className="text-xs font-medium text-gray-900">
                              {user.usage.auditsThisMonth}/{getCurrentPlan().limits.auditsPerMonth === 'unlimited' ? '∞' : getCurrentPlan().limits.auditsPerMonth}
                            </span>
                          </div>
                          <div className="w-full bg-gray-200 rounded-full h-1.5">
                            <div 
                              ...
                              style={{ 
                                width: getCurrentPlan().limits.auditsPerMonth === 'unlimited' 
                                  ? '100%' 
                                  : `${Math.min(100, (user.usage.auditsThisMonth / (getCurrentPlan().limits.auditsPerMonth as number)) * 100)}%` 
                              }}
                            ></div>
                          </div>
                          <p className="text-xs text-gray-500">
                            {getRemainingAudits() === 999999 
                              ? 'Audits illimités' 
                              : `${getRemainingAudits()} audit${getRemainingAudits() > 1 ? 's' : ''} restant${getRemainingAudits() > 1 ? 's' : ''}`
                            }
                          </p>
                        </div>
                      </div>
